defmodule Core.PRMRepo.Migrations.SetAdditionalInfoSpecialityOfficio1 do
  @moduledoc false
  ...
  def change do
    ...
    Enum.each(employees, fn [id, info, speciality_officio_name] ->
      ...
      {index, speciality_officio} =
        Enum.find(specialities, fn {_i, speciality} ->
      ...
      Employee
      |> where([e], e.id == ^id)
      |> PRMRepo.update_all(set: [additional_info: additional_info, speciality: nil])
    end)
  end
end
